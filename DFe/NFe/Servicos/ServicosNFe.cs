...
using DFe.NFe.Classes.Servicos.DistribuicaoDFe;
using DFe.NFe.Classes.Servicos.DistribuicaoDFe.Schemas;
using DFe.NFe.Classes.Servicos.Download;
using DFe.NFe.Classes.Servicos.Evento;
using DFe.NFe.Classes.Servicos.Inutilizacao;
using DFe.NFe.Classes.Servicos.Recepcao;
using DFe.NFe.Classes.Servicos.Recepcao.Retorno;
using DFe.NFe.Classes.Servicos.Status;
using DFe.NFe.Classes.Servicos.Tipos;
using DFe.NFe.Servicos.Retorno;
using DFe.NFe.Utils;
using DFe.NFe.Utils.AdmCsc;
using DFe.NFe.Utils.Autorizacao;
using DFe.NFe.Utils.Consulta;
using DFe.NFe.Utils.ConsultaCadastro;
using DFe.NFe.Utils.DistribuicaoDFe;
using DFe.NFe.Utils.Download;
using DFe.NFe.Utils.Evento;
using DFe.NFe.Utils.Excecoes;
using DFe.NFe.Utils.Inutilizacao;
using DFe.NFe.Utils.NFe;
using DFe.NFe.Utils.Recepcao;
using DFe.NFe.Utils.Status;
using DFe.NFe.Utils.Validacao;
using DFe.NFe.Wsdl;
using DFe.NFe.Wsdl.AdmCsc;
using DFe.NFe.Wsdl.Autorizacao;
using DFe.NFe.Wsdl.ConsultaCadastro.CE;
using DFe.NFe.Wsdl.ConsultaProtocolo;
using DFe.NFe.Wsdl.DistribuicaoDFe;
using DFe.NFe.Wsdl.Download;
using DFe.NFe.Wsdl.Evento;
using DFe.NFe.Wsdl.Inutilizacao;
using DFe.NFe.Wsdl.Recepcao;
using DFe.NFe.Wsdl.Status;
using DFe.Utils;
...
namespace DFe.NFe.Servicos
{
    public sealed class ServicosNFe : IDisposable
    {
        ...
        private INfeServico CriarServico(ServicoNFe servico)
        {
            ...
            switch (servico)
            {
                ...
                case ServicoNFe.NfeConsultaCadastro:
                    ...
                    return new DFe.NFe.Wsdl.ConsultaCadastro.DEMAIS_UFs.CadConsultaCadastro2(url, _certificado,
                        _cFgServico.TimeOut);
        ...
        /// <returns>Retorna um objeto da classe RetornoRecepcaoEvento com o retorno do serviço RecepcaoEvento</returns>
        public RetornoRecepcaoEvento RecepcaoEventoEpec(int idlote, int sequenciaEvento, DFe.NFe.Classes.NFe nfe,
            string veraplic)
        {
            var versaoServico =
                ...
                for (int i = 0; i < retConsulta.loteDistDFeInt.Length; i++)
                {
                    ...
                    else if (conteudo.StartsWith("<procEventoNFe"))
                    {
                        var procEventoNFeConteudo =
                            FuncoesXml.XmlStringParaClasse<DFe.NFe.Classes.Servicos.DistribuicaoDFe.Schemas.procEventoNFe>(
                                conteudo);
        ...
        public RetornoNfeRecepcao NfeRecepcao(int idLote, List<DFe.NFe.Classes.NFe> nFes)
        {
        ...
        public RetornoNFeAutorizacao NFeAutorizacao(int idLote, IndicadorSincronizacao indSinc, List<DFe.NFe.Classes.NFe> nFes,
