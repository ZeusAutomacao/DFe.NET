...
Há uma aplicação demonstrativa (NFe.Danfe.App.Teste.Html) em WPF com C# que gera um DANFE (Documento Auxiliar da Nota Fiscal Eletrônica) em formato HTML a partir de um arquivo XML de NFe. O arquivo gerado é salvo com a extensão .html e aberto automaticamente no navegador.
...
DanfeNFe danfe = new DanfeNFe(proc.NFe, Status.Autorizada, "55512121121211", "Emissor Fiscal Exemplo Brasil - www.exemplo.com.br");
...
```cs
public IActionResult GerarDanfeCce([FromBody] NotaFiscalCartaCorrecaoImprimirModel model)
{
...
Há uma aplicação de exemplo (NFe.Danfe.AppTeste.PdfClown) que demonstra como gerar o DANFE de uma NF-e em PDF utilizando a biblioteca **PDFClown**.
