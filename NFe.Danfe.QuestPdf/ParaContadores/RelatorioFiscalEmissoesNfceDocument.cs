...
public class RelatorioFiscalEmissoesNfceDocument : IDocument
{
    private static string Fonte = "Arial";
    private RelatorioFiscalEmissoesNfceModel Model;

    public RelatorioFiscalEmissoesNfceDocument(RelatorioFiscalEmissoesNfceModel model)
    {
        Model = model;
    }
    ...
    private void Cabecalho(IContainer container)
    {
        ...
        container.Row(row =>
        {
            ...
            {
                column.Item().AlignRight().Text("Relatório fiscal de emissões na NFC-e").FontSize(12).FontFamily(Fonte).SemiBold();
    ...
    private void Conteudo(IContainer container)
    {
        ...
        container
            ...
            .Column(column =>
            {
                TituloInicioTabela(column, "Documento Fiscal Modelo 65 - NFCE");

                column.Item().Element(TabelaNFCeResumida);
    ...
    private void TabelaNFCeResumida(IContainer container)
    {
        container.Table(table =>
        {
            ...
            foreach (var item in Model.NfceResumidaModels)
            {
    ...
    private void TabelaInutilizacoes(IContainer container)
    {
        container.Table(table =>
        {
            ...
            foreach (var item in Model.NfceInutilizacaoModels)
            {
    ...
    private void ResumoTotais(ColumnDescriptor column)
    {
        column.Item().Row(row =>
        {
            ...
            row.RelativeItem(3)
                ...
                {
                    text.AlignCenter();
                    text.Span("Cupons").FontSize(10).FontFamily(Fonte).SemiBold().Italic();
                });
        });

        column.Item().Row(LinhaTotal("Resumo notas autorizadas"
            , Model.QuantidadeNfceAutorizada
            , Model.ValorTotalNfceAutorizada));

        column.Item().Row(LinhaTotal("Resumo notas canceladas"
            , Model.QuantidadeNfceCancelada
            , Model.ValorTotalNfceCancelada));

        column.Item().Row(LinhaTotal("Resumo notas denegadas"
            , Model.QuantidadeNfceDenegada
            , Model.ValorTotalNfceDenegada));


    }

    private Action<RowDescriptor> LinhaTotal(string descricaoLinha, decimal quantidadeNfce, decimal valorTotal)
    {
        return row =>
        {
            row.RelativeItem(7)
